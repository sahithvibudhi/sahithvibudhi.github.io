...
const skills = ['HTML & (S)CSS', 'Bootstrap', 'Javascript', 'JQuery', 'Typescript', 'React', 'Express', 'Python + Flask', 'Java', 'GoLang', 'PHP', 'MySQL', 'MongoDB', 'Elastic Search', 'Redis', 'Docker', 'Kubernetes', 'AWS', 'Apache Kafka', 'GraphQL', 'gRPC', 'Git', 'Spring MVC', 'Yii MVC'];
...
export default () => {
    return <div className='container mt-5 about'>
            ...
            <ul className='row'>
                {skills.map(val => <li className='col-xs-6 col-sm-4 col-md-2' style={skillListStyle}>{val}</li>)}
            </ul>
        </div>;
}